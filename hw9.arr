...
#|
   ...
   Bonus:
   - endgame: 1/1
   *** I see that you have done the functions for when the player loses or wins, but you forgot to implement them into the reactor
   - stopping: 1/1
   - generate board: 0/1
 
   
   For next week, try to write cant-move-left, cant-move-right, vertical-helper, color-helper  as instances of one general function,
   by writing something like contains from lab. 
